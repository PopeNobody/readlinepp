...
/* Set to non-zero by calling application if it has already printed
   rl_prompt and does not want readline to do it the first time. */
...
#define RESIZE_KEYSEQ_BUFFER()                                            \
  ...
  {                                                                       \
    ...
    {                                                                     \
      ...
      rl_executing_keyseq= (char*)xrealloc(rl_executing_keyseq,           \
...
/* Non-zero means to prefix the displayed prompt with a character
   indicating the editing mode: @ for emacs, : for vi-command, + for
   vi-insert. */
...
using std::string;
std::pair<bool,str_t> readline(str_c prompt)
{
  char* value;
#if 0
  int in_callback;
  ...
  if(!value)
    return make_pair(false,str_t());
  return make_pair(true,str_t(value));
}
...
STATIC_CALLBACK char* readline_internal_teardown(int eof)
{
  ...
  if (entry && rl_undo_list)
  {
    ...
    rl_revert_line(1, 0);
    entry= replace_history_entry(
             where_history(), the_line, (histdata_t)NULL);
  ...
  /* Disable the meta key, if this terminal has one and we were told to use
     it. The check whether or not we sent the enable string is in
...
#endif
{
 ...
 will be returned; punt immediately on read error without converting it to
...
return (eof_found);
  ...
  int _rl_dispatch_callback(_rl_keyseq_cxt * cxt)
  {
    ...
    /* For now */
    /* The first time this context is used, we want to read input and
       dispatch on it.  When traversing the chain of contexts back `up', we
       want to use the value from the next context down.  We're simulating
       recursion using a chain of contexts. */
    ...
    if (r !=
      ...
      r= _rl_subseq_result(
               r, cxt->oldmap, cxt->okey, (cxt->flags & KSEQ_SUBSEQ));

    RL_CHECK_SIGNALS();
    /* We only treat values < 0 specially to simulate recursion. */
    if (r >= 0 || (r == -1 && (cxt->flags & KSEQ_SUBSEQ) ==
    ...
    switch (map[key].type)
    {
      case ISFUNC:
        ...
        if (func)
        {
          ...
          if (rl_pending_input == 0 &&
              map[key].function != rl_digit_argument)
      ...
      case ISKMAP:
        ...
        {
#if defined(VI_MODE)
          /* The only way this test will be true is if a subsequence has
             been bound starting with ESC, generally the arrow keys.  What
             we do is check whether there's input in the queue, which there
             ...
             test with a zero timeout (by
             default) or a timeout determined by the value of
             `keyseq-timeout' */
          ...
          if (rl_editing_mode == vi_mode && key == ESC &&
              ...
              _rl_input_queued((_rl_keyseq_timeout > 0)
                                        ...
                                        : 0) == 0)
            return (_rl_dispatch(ANYOTHERKEY,
                                 FUNCTION_TO_KEYMAP(map, key)));
          /* This is a very specific test.  It can possibly be generalized
             in the future, but for now it handles a specific case of ESC
             being the last character in a keyboard macro. */
          if (rl_editing_mode == vi_mode && key == ESC &&
              map == vi_insertion_keymap &&
          ...
          /* Allocate new context here.  Use linked contexts (linked
             through cxt->ocxt) to simulate recursion */
#if defined(READLINE_CALLBACKS)
#if defined(VI_MODE)
          /* If we're redoing a vi mode command and we know there is a
             shadowed function corresponding to this key, just call it --
             all the redoable vi mode commands already have all the input
             they need, and rl_vi_redo assumes that one call to rl_dispatch
             is sufficient to complete the command. */
  ...
  static void bind_arrow_keys(void)
  {
    ...
    if (vi_movement_keymap[ESC].type == ISKMAP)
      rl_bind_keyseq_in_map(
               "\033", (rl_command_func_t*)NULL, vi_movement_keymap);
