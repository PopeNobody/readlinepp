...
add_character:
  ...
  switch (cxt->lastc)
  {
    ...
    case -1:
      if (cxt->search_string_index == 0)
      {
        if (last_isearch_string)
        {
          ...
          cxt->search_string     = (char*)xrealloc(cxt->search_string,
                                              cxt->search_string_size);
   ...
   do until we have a real isearch-undo. */
    ...
    case -7: /* bracketed paste */
      ...
      if (cxt->search_string_index + pastelen + 1 >=
          cxt->search_string_size)
      {
 ...
 searching for. */
    ...
    cxt->sline_index= (cxt->sflags & SF_REVERSE)
...
/* Called from the callback functions when we are ready to read a key.  The
   callback functions know to call this because
   RL_ISSTATE(RL_STATE_ISEARCH). If _rl_isearch_dispatch finishes
   searching, this function is responsible for turning off
   RL_STATE_ISEARCH, which it does using _rl_isearch_cleanup. */
