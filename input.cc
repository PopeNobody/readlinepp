...
/* Windows isatty returns true for every character device, including the
   null device, so we need to perform additional checks. */
...
int rl_read_key(void)
{
  ...
  else
  {
    ...
    else
    {
       ...
       * _rl_caught_signal = %d", _rl_caught_signal); */
      RL_CHECK_SIGNALS();
    }
  }
...
int rl_getc(FILE* stream)
{
  ...
  while (1)
  {
    ...
    result= pselect(fileno(stream) + 1,
                    &readfds,
                    NULL,
                    NULL,
                    NULL,
    ...
    result= pselect(
             fileno(stream) + 1, &readfds, NULL, NULL, NULL, &empty_set);
