...
typedef str_t &str_r;
typedef const str_t &str_c;
class str_v : public std::vector<str_t>
{
  public:
  typedef std::string string;
  typedef std::vector<string> string_v;
  typedef std::vector<str_t> super;
  str_v()
  {
  };
  template<typename itr_t>
    str_v(itr_t lhs, itr_t rhs)
    : super(lhs,rhs)
    {
    };
  str_v(const super &rhs)
    : super(rhs)
  {
  };
  str_v(const string_v &rhs)
    : super(rhs.begin(),rhs.end())
  {
  };
  str_v &operator=(const string_v &rhs)
  {
    str_v tmp(rhs.begin(),rhs.end());
    swap(tmp);
    return *this;
  };

  str_v &operator=(const super &rhs)
  {
    str_v tmp(rhs.begin(),rhs.end());
    swap(tmp);
    return *this;
...
extern std::pair<bool,str_t> readline(const str_t &line);
