...
static string noninc_search_string;
static int   noninc_history_pos;

static string prev_line_found;

static int rl_history_search_len;
static int rl_history_search_pos;
static int rl_history_search_flags;

static string history_search_string;

static void make_history_line_current (HIST_ENTRY*);
static int noninc_search_from_pos     (const string &, int, int, int, int*);
static int noninc_dosearch            (const string &, int, int);
...
{
  _rl_replace_text(entry->line.c_str(), 0, rl_end);
  _rl_fix_point(1);
#if defined(VI_MODE)
...
static int noninc_search_from_pos(const string &str,
                                  int   pos,
                                  int   dir,
                                  int   flags,
                                  int*  ncp)
{
  ...
  /* These functions return the match offset in the line; history_offset
     gives the matching line in the history list */
  if (flags & SF_PATTERN)
  {
    string s     = str;
...
static int noninc_dosearch(char* string, int dir, int flags)
{
  ...
  pos= noninc_search_from_pos(
           string, noninc_history_pos + dir, dir, flags, (int*)0);
  if (pos == -1)
  {
...
/* Process just-read character C according to search context CXT.  Return
   -1 if the caller should abort the search, 0 if we should break out of
   the loop, and 1 if we should continue to read characters. */
...
static int _rl_nsearch_dosearch(_rl_search_cxt* cxt)
{
 ...
 fails, the old undo list will be restored by rl_maybe_unsave_line.) */
  ...
  rl_restore_prompt();
  return (noninc_dosearch(noninc_search_string,
                          cxt->direction,
                          cxt->sflags & SF_PATTERN));
}
...
static int rl_history_search_internal(int count, int dir)
{
  ...
  {
    RL_CHECK_SIGNALS();
    ret= noninc_search_from_pos(history_search_string,
                                rl_history_search_pos + dir,
                                dir,
                                0,
                                &newcol);
    if (ret == -1)
    ...
    if (prev_line_found.length() && prev_line_found == temp->line.c_str())
      continue;
...
static void rl_history_search_reinit(int flags)
{
  ...
  if (rl_point)
  {
    ...
    history_search_string += rl_line_buffer;
  }
