...
static void hist_string_extract_single_quoted(char* string,
                                              int*  sindex,
                                              int   flags)
{
   int i;
...
static char* get_subst_pattern(char* str,
                               int*  iptr,
                               int   delimiter,
                               int   is_rhs,
                               int*  lenptr)
  ...
  for (i= j= 0; i < subst_rhs_len; i++)
  {
    if (subst_rhs[i] == '&')
    {
      if (j + subst_lhs_len >= new_size)
        _new= (char*)xrealloc(_new,
                              (new_size= new_size * 2 + subst_lhs_len));
...
static int history_expand_internal(char*  string,
                                   int    start,
                                   int    qc,
                                   int*   end_index_ptr,
                                   char** ret_string,
...
int history_expand(char* hstring, char** output)
{
  ...
  /* The quick substitution character is a history expansion all right.
     That is to say, "^this^that^" is equivalent to "!!:s^this^that^", and
     in fact, that is the substitution that we do. */
...
static char* get_history_word_specifier(char* spec,
                                        char* from,
                                        int*  caller_index)
{
   int i= *caller_index;
...
/* Parse STRING into tokens and return an array of strings.  If WIND is
   ...
   *INDP. */
static char** history_tokenize_internal(const char* string,
                                        int         wind,
                                        int*        indp)
{
  char**       result;
