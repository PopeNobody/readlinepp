...
/* If we're compiling for __EMX__ (OS/2) or __CYGWIN__ (cygwin32
   environment on win 95/98/nt), we want to open files with O_BINARY mode
...
/* If non-zero, we write timestamps to the history file in
 * history_do_write() */
...
static char* history_tempfile(const char* filename)
{
  ...
  fn= filename;
#if defined(HAVE_READLINK)
  /* Follow symlink so tempfile created in the same directory as any
     symlinked history file; call will fail if not a symlink */
  if ((n= readlink(filename, linkbuf, sizeof(linkbuf) - 1)) > 0)
  {
    linkbuf[n]= '\0';
    fn        = linkbuf;
...
int read_history_range(const char* filename, int from, int to)
{
...
#ifdef HISTORY_USE_MMAP
  /* We map read/write and private so we can change newlines to NULs
     without affecting the underlying object. */
  buffer= (char*)mmap(
           0, file_size, PROT_READ | PROT_WRITE, MAP_RFLAGS, file, 0);
  ...
  *bufend     = '\0'; /* null-terminate buffer for timestamp checks */
  ...
  for (line_start= line_end= buffer;
       line_end < bufend && current_line < from;
       line_end++)
    if (*line_end == '\n')
    {
      p= line_end + 1;
...
truncate_exit:
  ...
  /* Make sure the new filename is owned by the same user as the old.  If
     one user is running this, it's a no-op.  If the shell is running after
     sudo with a shared history file, we don't want to leave the history
     file owned by root. */
  if (rv == 0 && exists)
...
/* Workhorse function for writing history.  Writes the last NELEMENT
   entries from the history list to FILENAME.  OVERWRITE is non-zero if you
   wish to replace FILENAME with the entries. */
static int history_do_write(const char* filename,
                            int         nelements,
                            int         overwrite)
{
   int i;
  ...
  /* Build a buffer of all the lines to write, and write them in one
