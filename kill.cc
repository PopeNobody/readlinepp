...
/* Add TEXT to the kill ring, allocating a _new kill ring slot as
   necessary. This uses TEXT directly, so the caller must not free it.  If
   APPEND is non-zero, and the last command was a kill, the text is
   appended to the current kill ring slot, otherwise prepended. */
static int _rl_copy_to_kill_ring(char* text, int append)
{
  ...
  if (_rl_last_command_was_kill == 0 || rl_kill_ring == 0)
  {
    ...
    if (rl_kill_ring == 0)
    {
      ...
      rl_kill_ring= (char**)xmalloc(((rl_kill_ring_length= 1) + 1) *
                                    sizeof(char*));
    ...
    else
    {
      ...
      else
      {
        ...
        rl_kill_ring= (char**)xrealloc(rl_kill_ring,
                                       (slot + 1) * sizeof(char*));
      }
...
static int region_kill_internal(int _delete)
{
  char* text;
...
char* _rl_bracketed_text(size_t* lenp)
{
  ...
  while ((c= rl_read_key()) >= 0)
  {
    ...
    if (len >= BRACK_PASTE_SLEN && c == BRACK_PASTE_LAST &&
        STREQN(buf + len - BRACK_PASTE_SLEN,
               BRACK_PASTE_SUFF,
               BRACK_PASTE_SLEN))
    {
