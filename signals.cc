...
static SigHandler* rl_set_sighandler    PARAMS((int,
                                             SigHandler*,
                                             sighandler_cxt*));
static void rl_maybe_set_sighandler     PARAMS((int,
                                            SigHandler*,
                                            sighandler_cxt*));
...
/* If non-zero, readline will install its own signal handlers for
   SIGINT, SIGTERM, SIGHUP, SIGQUIT, SIGALRM, SIGTSTP, SIGTTIN, and
   SIGTTOU. */
...
/* If non-zero, print characters corresponding to received signals as long
   as
...
static RETSIGTYPE _rl_handle_signal(int sig)
{
#if defined(HAVE_POSIX_SIGNALS)
  sigset_t set;
  ...
  /* If there's a sig cleanup function ed, call it and `deregister'
...
#if defined(SIGWINCH)
static RETSIGTYPE rl_sigwinch_handler(int sig)
{
  SigHandler* oh;
                                     ...
                                     sighandler_cxt* ohandler)
{
  ...
  /* If rl_set_signals is called twice in a row, don't set the old handler
     to rl_signal_handler, because that would cause infinite recursion. */
...
/* Set disposition of SIG to HANDLER, returning old state in OHANDLER.
   Don't change disposition if OHANDLER indicates the signal was ignored.
 */
...
/* Set the disposition of SIG to HANDLER, if HANDLER->sa_handler indicates
   the signal was not being ignored.  MUST only be called for signals whose
...
/* Free up the readline variable line state for the current line (undo
   list, any partial history entry, any keyboard macros in progress, and
   any numeric arguments in process) after catching a signal, before
   calling rl_cleanup_after_signal(). */
