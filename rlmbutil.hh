...
#include <wctype.h>
         ...
         defined(HAVE_MBRTOWC) && defined(HAVE_MBRLEN) &&                 \
         defined(HAVE_TOWLOWER) && defined(HAVE_TOWUPPER) &&              \
...
/* If we don't want multibyte chars even on a system that supports them,
   let the configuring user turn multibyte support off. */
...
extern int _rl_compare_chars
                            PARAMS((char*, int, mbstate_t*, char*, int, mbstate_t*));
...
/* Try and shortcut the printable ascii characters to cut down the number
   of calls to a libc wcwidth() */
