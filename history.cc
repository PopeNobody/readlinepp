...
/* Return the current history array.  The caller has to be careful, since
   this is the actual array of data, and could be bashed or made corrupt
   easily. The array is terminated with a NULL pointer. */
...
time_t history_get_time(HIST_ENTRY* hist)
{
  ...
  errno= 0;
  t    = (time_t)strtol(
           ts + 1, (char**)NULL, 10); /* XXX - should use strtol() here */
  if (errno == ERANGE)
    return (time_t)0;
  return t;
}
...
HIST_ENTRY** remove_history_range(int first, int last)
{
  ...
  if (the_history == 0 || history_length == 0)
    return ((HIST_ENTRY**)NULL);
  if (first < 0 || first >= history_length || last < 0 ||
      last >= history_length)
    return ((HIST_ENTRY**)NULL);
  if (first > last)
    return (HIST_ENTRY**)NULL;
  ...
  /* Copy the rest of the entries, moving down NENTRIES slots.  Copy
