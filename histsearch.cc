...
static int history_search_internal(const char* string,
                                   int         direction,
                                   int         flags)
{
   int   i, reverse;
...
int _hs_history_patsearch(const char* string, int direction, int flags)
{
  ...
  /* If the search string is not anchored, we'll be calling fnmatch
     (assuming we have it). Prefix a `*' to the front of the search string
     so we search anywhere in the line. */
