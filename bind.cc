...
const char* const _rl_possible_control_prefixes[]= { "Control-",
                                                     "C-",
                                                     "CTRL-",
                                                     (const char*)NULL };

const char* const _rl_possible_meta_prefixes[]= { "Meta",
                                                  "M-",
                                                  (const char*)NULL };
...
int rl_parse_and_bind(char* string)
{
  ...
  /* Advance to the colon (:) or whitespace which separates the two
   * objects. */
...
static int sv_bell_style(const char* value)
{
  if (value == 0 || *value == '\0')
    _rl_bell_preference= AUDIBLE_BELL;
  else if (_rl_stricmp(value, "none") == 0 ||
           _rl_stricmp(value, "off") == 0)
...
/* -1 for NULL entry */
#define NUM_BUILTIN_KEYMAPS                                               \
  (sizeof(builtin_keymap_names) / sizeof(builtin_keymap_names[0]) - 1)
...
int rl_set_keymap_name(const char* name, Keymap map)
{
  ...
  {
    keymap_names= (name_and_keymap*)xmalloc(
    ...
    keymap_names= (name_and_keymap*)xrealloc(
             keymap_names, (i + 2) * sizeof(struct name_and_keymap));
  ...
  for (key= 0; key < KEYMAP_SIZE; key++)
  {
    switch (map[key].type)
    {
      ...
      case ISKMAP:
      {
        ...
        if (map[key].function)
          seqs= rl_invoking_keyseqs_in_map(function,
    ...
    switch (map[key].type)
    {
      ...
      case ISKMAP:
        ...
        _rl_macro_dumper_internal(
                 print_readably, FUNCTION_TO_KEYMAP(map, key), keyname);
        xfree(keyname);
        break;
    }
  }
...
static char* _rl_get_string_variable_value(const char* name)
{
  ...
  if (_rl_stricmp(name, "bell-style") == 0)
  {
    ...
    return (char*)(_rl_comment_begin ? _rl_comment_begin
  ...
  else if (_rl_stricmp(name, "keyseq-timeout") == 0)
  {
    ...
    return (char*)(_rl_vi_cmd_mode_str ? _rl_vi_cmd_mode_str
                                       : RL_VI_CMD_MODESTR_DEFAULT);
  else if (_rl_stricmp(name, "vi-ins-mode-string") == 0)
    return (char*)(_rl_vi_ins_mode_str ? _rl_vi_ins_mode_str
...
static int substring_member_of_array(const char*        string,
