...
#define DPRINT2(FMT, V1, V2)                                              \
  (fprintf(logfile, FMT, V1, V2), fflush(logfile))
#else
...
static void add_special_char(int ch)
{
  if (ch != 0)
...
{
  if (on == current_emphasize_input || (on && !do_emphasize_input))
    return;
  if (on)
  {
...
static void null_prep_terminal(int meta)
{
}

static void null_deprep_terminal()
{
  maybe_emphasize_input(0);
}

static int pre_input_change_mode(void)
{
  return 0;
}
...
static void line_handler(char* line)
{
  ...
  else
  {
    ...
    else
    {
      ...
      memcpy(echo_suppress_buffer + echo_suppress_limit, line, length);
...
int main(int argc, char** argv)
{
  ...
  t.c_lflag&= ~(ICANON | ISIG | ECHO | ECHOCTL | ECHOE | ECHOK | ECHONL
#if defined(ECHOKE)
                | ECHOKE
#endif
#if defined(ECHOPRT)
  ...
  for (;;)
  {
    ...
    if (FD_ISSET(in_from_tty_fd, &in_set))
    {
      ...
      if (ioctl_ret >= 0)
      {
        ...
        DPRINT1("echo,canon,crnl:%03d\n",
                100 * _rl_echoing_p + 10 * do_canon + 1 * do_icrnl);
      }
      else
      {
        if (ioctl_err == 0)
    ...
    else /* output from inferior. */
    {
      ...
      if (buf_count > (sizeof(buf) >> 2))
        buf_count= 0;
      count= read(in_from_inferior_fd,
                  buf + buf_count,
      ...
      while (echo_suppress_start < echo_suppress_limit && count > 0 &&
