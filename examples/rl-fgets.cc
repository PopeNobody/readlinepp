...
*******************************************************************************
  ...
  ABSTRACT:  Customize fgets() behavior via LD_PRELOAD in the following
ways:
    ...
    -- A list of commands for command-name completion can be configured by
       setting the environment-variable FGETS_COMMAND_FILE to a file
containing the list of commands to be used.

    -- Command-line editing with readline() works best when the prompt
string is known; you can set this with the FGETS_PROMPT environment
variable.

    -- There special strings that libfgets will interpret as internal
commands:
  ...
  AIX:   gcc -fPIC fgets.c -shared -o libfgets.so -lc -ldl -lreadline
-ltermcap HP-UX: gcc -fPIC fgets.c -shared -o libfgets.so -lc -ldld
-lreadline Linux: gcc -fPIC fgets.c -shared -o libfgets.so -lc -ldl
-lreadline SunOS: gcc -fPIC fgets.c -shared -o libfgets.so -lc -ldl -lgen
-lreadline
  ...
  for the LD_PRELOAD concept.  The generic method for 32-bit platforms is
to put libtermcap.so, libfgets.so, and libreadline.so (with absolute paths)
  in the LD_PRELOAD environment variable, and to put their parent
directories in the LD_LIBRARY_PATH environment variable.  Unfortunately
there is no generic method for 64-bit platforms; e.g. for 64-bit SunOS, you
would have to build both 32-bit and 64-bit libfgets and libreadline
libraries, and use the LD_FLAGS_32 and LD_FLAGS_64 environment variables
with preload and library_path configurations (a mix of 32-bit and 64-bit
calls are made under 64-bit SunOS).
...
/* -- writeable data is stored in the shared library's data segment
   -- every process that uses the shared library gets a private memory copy
   of its entire data segment
...
static void my_fgets_reset(void)
{
...
static void my_fgets_dump(void)
{
  char* s;
  printf("\n");
...
static void my_fgets_debug_toggle(void)
{
...
static char* my_fgets_lookup(int index)
{
...
static char* my_fgets_generator(const char* text, int state)
{
...
static char** my_fgets_completion(const char* text, int start, int end)
{
  char** matches;
  matches= NULL;
  if (!start)
...
char* fgets(char* s, int n, FILE* stream)
{
  ...
  if (stream == stdin)
  {
    ...
    for (my_fgets_line= 0; !my_fgets_line; my_fgets_line= readline(prompt))
      ;
