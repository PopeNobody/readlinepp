...
static int _rl_utf8_mblen(const char* s, size_t n)
{
  ...
  if (c >= 0xc2)
  {
    ...
    {
      if (n >= 3 && (s[1] ^ 0x80) < 0x40 && (s[2] ^ 0x80) < 0x40 &&
          (c >= 0xe1 || c1 >= 0xa0) && (c != 0xed || c1 < 0xa0))
        return 3;
    }
    else if (c < 0xf8)
    {
      if (n >= 4 && (s[1] ^ 0x80) < 0x40 && (s[2] ^ 0x80) < 0x40 &&
          (s[3] ^ 0x80) < 0x40 && (c >= 0xf1 || c1 >= 0x90) &&
...
static int _rl_find_next_mbchar_internal(char* string,
                                         int   seed,
                                         int   count,
                                         int   find_non_zero)
{
...
/*static*/ int _rl_find_prev_mbchar_internal(char* string,
                                             int   seed,
                                             int   find_non_zero)
{
  mbstate_t ps;
...
/* return the number of bytes parsed from the multibyte sequence starting
   ...
   if an invalid multibyte sequence was encountered. It returns
...
int _rl_compare_chars(char*      buf1,
                      int        pos1,
                      mbstate_t* ps1,
                      char*      buf2,
                      int        pos2,
...
int _rl_is_mbchar_matched(char* string,
                          int   seed,
                          int   end,
                          char* mbchar,
                          int   length)
