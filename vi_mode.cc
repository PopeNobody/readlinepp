...
/* This set of functions is basically to handle the commands that take a
   motion argument while in callback mode: read the command, read the
   motion command modifier, find the extent of the text to affect, and
...
static int vi_delete_dispatch(_rl_vimotion_cxt* m)
{
  /* These are the motion commands that do not require adjusting the
     mark. */
  if (((strchr(" l|h^0bBFT`", m->motion) == 0) &&
       (rl_point >= m->start)) &&
      (rl_mark < rl_end))
...
int rl_vi_delete_to(int count, int key)
{
  ...
  if (_rl_uppercase_p(key))
  {
    ...
    r                  = rl_domove_motion_callback(_rl_vimvcxt);
           ...
           _rl_vi_last_motion != 'd') /* `dd' is special */
  {
...
static int vi_change_dispatch(_rl_vimotion_cxt* m)
{
  ...
  if (((strchr(" l|hwW^0bBFT`", m->motion) == 0) &&
       (rl_point >= m->start)) &&
      (rl_mark < rl_end))
    INCREMENT_POS(rl_mark);
...
int rl_vi_change_to(int count, int key)
{
  ...
  if (_rl_uppercase_p(key))
  {
    ...
    r                  = rl_domove_motion_callback(_rl_vimvcxt);
           ...
           _rl_vi_last_motion != 'c') /* `cc' is special */
  {
    _rl_vimvcxt->motion= _rl_vi_last_motion;
    r                  = rl_domove_motion_callback(_rl_vimvcxt);
  }
...
static int vi_yank_dispatch(_rl_vimotion_cxt* m)
{
  /* These are the motion commands that do not require adjusting the
     mark. */
  if (((strchr(" l|h^0%bBFT`", m->motion) == 0) &&
       (rl_point >= m->start)) &&
      (rl_mark < rl_end))
    INCREMENT_POS(rl_mark);
...
int rl_vi_yank_to(int count, int key)
{
  ...
  if (_rl_uppercase_p(key))
  {
    ...
    r                  = rl_domove_motion_callback(_rl_vimvcxt);
           ...
           _rl_vi_last_motion != 'y') /* `yy' is special */
  {
    _rl_vimvcxt->motion= _rl_vi_last_motion;
    r                  = rl_domove_motion_callback(_rl_vimvcxt);
  }
...
#if defined(HANDLE_MULTIBYTE)
  return (_rl_char_search_internal(data->count,
                                   _rl_cs_dir,
                                   _rl_vi_last_search_mbchar,
                                   _rl_vi_last_search_mblen));
#else
  return (_rl_char_search_internal(
           data->count, _rl_cs_dir, _rl_vi_last_search_char));
#endif
}
#endif
...
static int _rl_vi_goto_mark(void)
{
  ...
  if (ch == '`')
  {
    ...
    return 0;
           ...
           ch > 'z') /* make test against 0 explicit */
  {
    rl_ding();
    return 1;
  }
