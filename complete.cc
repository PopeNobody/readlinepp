...
/* If non-zero, then this is the address of a function to call when
   ...
   NUM_MATCHES is the number of strings in that array, and MAX_LENGTH is
   the length of the longest string in that array. */
...
static char** gen_completion_matches
         PARAMS((char*, int, int, rl_compentry_func_t*, int, int));
...
/* If non-zero, the symlinked directory completion behavior introduced in
   ...
   _rl_complete_mark_directories). This is user-settable via the
   mark-symlinked-directories variable. */
...
/* Length in characters of a common prefix replaced with an ellipsis
   (`...') when displaying completion matches.  Matches whose printable
   portion has more than this number of displaying characters in common
   will have the common display prefix replaced with an ellipsis. */
...
/* Pointer to generator function for rl_menu_complete ().  NULL means to
 *use rl_completion_entry_function (see above). */
...
rl_compignore_func_t* rl_ignore_some_completions_function=
         (rl_compignore_func_t*)NULL;

/* Set to a function to quote a filename in an application-specific
   fashion. Called with the text to quote, the type of match found (single
   or multiple) and a pointer to the quoting character to be used, which
   the function can reset if desired. */
...
/* Set to any quote character readline thinks it finds before any
   application completion function is called. */
int rl_completion_quote_character;

/* Set to a non-zero value if readline found quoting anywhere in the word
   to be completed; set before any application completion function is
   called. */
...
/* Set default values for readline word completion.  These are the
   variables that application completion functions can change or inspect.
 */
...
static int get_y_or_n(int for_pager)
{
  ...
  /* For now, disable pager in callback mode, until we later convert to
     state driven functions.  Have to wait until next major version to add
     new state definition, since it will change value of RL_STATE_DONE. */
...
static char* printable_part(char* pathname)
{
  ...
  if (temp == 0 && ISALPHA((unsigned char)pathname[0]) &&
      pathname[1] == ':')
    temp= pathname + 1;
#endif
...
static int fnprint(const char* to_print,
                   int         prefix_bytes,
...
static int print_filename(char* to_print,
                          char* full_pathname,
                          int   prefix_bytes)
{
  ...
  if (scan)
  {
    ...
    if (isbrk)
    {
      ...
      if (rl_basic_quote_characters &&
          strchr(rl_basic_quote_characters, scan) && (end - rl_point) > 1)
        delimiter= scan;

      /* If the character isn't needed to determine something special
         about what kind of completion to perform, then advance past it. */
      if (rl_special_prefixes == 0 ||
          strchr(rl_special_prefixes, scan) == 0)
        rl_point++;
    }
  }
...
/* Find the common prefix of the list of matches, and put it into
   matches[0]. */
static int compute_lcd_of_matches(char**      match_list,
                                  int         matches,
                                  const char* text)
{
  ...
  for (i= 1, low= 100000; i < matches; i++)
  {
    ...
    {
      for (si= 0; (c1= _rl_to_lower(match_list[i][si])) &&
        ...
        if (MB_CUR_MAX > 1 && rl_byte_oriented == 0)
        {
          v1= mbrtowc(&wc1,
                      match_list[i] + si,
                      strlen(match_list[i] + si),
                      &ps1);
          v2= mbrtowc(&wc2,
                      match_list[i + 1] + si,
                      strlen(match_list[i + 1] + si),
                      &ps2);
    ...
    else
    {
      for (si= 0; (c1= match_list[i][si]) && (c2= match_list[i + 1][si]);
        ...
        if (MB_CUR_MAX > 1 && rl_byte_oriented == 0)
        {
          mbstate_t ps_back;
          ps_back= ps1;
          if (!_rl_compare_chars(match_list[i],
                                 si,
                                 &ps1,
                                 match_list[i + 1],
                                 si,
 ...
 the user typed in the face of multiple matches differing in case. */
    if (_rl_completion_case_fold)
    {
      ...
      dtext= (char*)NULL;
      if (rl_filename_completion_desired &&
          rl_filename_dequoting_function && rl_completion_found_quote &&
...
void rl_display_match_list(char** matches, int len, int max)
{
  ...
  else if (_rl_colored_completion_prefix > 0)
  {
    ...
    temp         = rl_filename_completion_desired ? strrchr(t, '/') : 0;
    common_length= temp ? fnwidth(temp) : fnwidth(t);
    sind         = temp ? RL_STRLEN(temp + 1)
               : RL_STRLEN(t); /* want portion after final slash */
    if (common_length > max || sind > max)
      common_length= sind= 0;
  }
#endif
 ...
 matches needs to be quoted. */
    should_quote=
             rl_filename_quote_characters
                      ? (_rl_strpbrk(match,
                                     rl_filename_quote_characters) != 0)
                      : 0;
...
static void insert_match(char* match, int start, int mtype, char* qc)
{
 ...
 the opening quote and insert the (fully-quoted) replacement. */
    else if (qc && (*qc != oqc) && start &&
             rl_line_buffer[start - 1] == oqc && replacement[0] != oqc)
      start--;
    end= rl_point - 1;
...
static int append_to_match(char* text,
                           int   delimiter,
                           int   quote_char,
...
/* Complete the word at or before point.
   ...
   `@' means to do standard completion, and list all possible completions
   if there is more than one and partial completion is not possible. */
int rl_complete_internal(int what_to_do)
{
  ...
  saved_line_buffer=
           rl_line_buffer ? savestring(rl_line_buffer) : (char*)NULL;
  ...
  matches= gen_completion_matches(
  ...
  if (saved_line_buffer)
  {
    completion_changed_buffer=
             strcmp(rl_line_buffer, saved_line_buffer) != 0;
 ...
 */
static int complete_fncmp(const char* convfn,
                          int         convlen,
                          const char* filename,
                          int         filename_len)
{
...
char* rl_filename_completion_function(const char* text, int state)
{
 ...
 all of its contents, close it. */
    ...
    if (dirname[0] == '/' && dirname[1] == '/' &&
        ISALPHA((unsigned char)dirname[2]) && dirname[3] == '/')
 ...
 it if necessary. */
    if (rl_completion_found_quote && rl_filename_dequoting_function)
      users_dirname= (*rl_filename_dequoting_function)(
               dirname, rl_completion_quote_character);
    else
 ...
 be empty. */
    if (*filename && rl_completion_found_quote &&
        rl_filename_dequoting_function)
    {
      /* delete single and double quotes */
      temp= (*rl_filename_dequoting_function)(
               filename, rl_completion_quote_character);
      xfree(filename);
      filename= temp;
    }
  ...
  /* At this point we should entertain the possibility of hacking
     wildcarded filenames, like /usr/man/man<WILD>/te<TAB>.  If the
     directory name contains globbing characters, then build an array of
     directories, and then map over that list while completing. */
  /* *** UNIMPLEMENTED *** */
...
/* An initial implementation of a menu completion function a la tcsh.  The
   first time (if the last readline command was not rl_old_menu_complete),
   we generate the list of matches.  This code is very similar to the code
   in rl_complete_internal -- there should be a way to combine the two.
   Then, for each item in the list of matches, we insert the match in an
   undoable fashion, with the appropriate character appended (this happens
   on the second and subsequent consecutive calls to rl_old_menu_complete).
   When we hit the end of the match list, we restore the original unmatched
   text, ring the bell, and reset the counter to zero. */
 ...
 code below should take care of it. */
  ...
  else
  {
    insert_match(matches[match_list_index],
                 ...
                 SINGLE_MATCH,
                 &quote_char);
    append_to_match(matches[match_list_index],
                    delimiter,
                    quote_char,
