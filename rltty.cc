...
/* Non-zero means that the terminal is in a prepped state.  There are
   several flags that are OR'd in to denote whether or not we have sent
   various init strings to the terminal. */
...
#if defined(TIOCGETC)
  struct tchars
                 tchars; /* Terminal special characters, including ^S and ^Q. */
#endif
#if defined(TIOCGLTC)
...
static void set_special_char
         PARAMS((Keymap, TIOTYPE*, int, rl_command_func_t*));
...
static void prepare_terminal_settings(int      meta_flag,
                                      TIOTYPE  oldtio,
                                      TIOTYPE* tiop)
{
                                      ...
                                      TIOTYPE* tiop)
{
  ...
  /* Conditionally disable some other tty special characters if there is a
     key binding for them in the current keymap.  Readline ordinarily
     doesn't bind these characters, but an application or user might. */
#if defined(VI_MODE)
...
/* Set readline's idea of whether or not it is echoing output to the
   terminal, returning the old value. */
...
#elif defined(NEW_TTY_DRIVER)
static void set_special_char(Keymap             kmap,
                             TIOTYPE*           tiop,
                             int                sc,
...
/* Set the system's default editing characters to their readline
   equivalents in KMAP.  Should be static, now that we have
